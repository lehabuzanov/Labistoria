...
def card(title: str, body: str) -> None:
    st.markdown(
        ...
        <div class="metric-card">
          <div style="font-size:0.95rem;color:#765638;text-transform:uppercase;letter-spacing:0.12em;">{title}</div>
          <div style="font-size:1.35rem;font-weight:600;margin-top:0.25rem;">{body}</div>
        </div>
        """,
...
def render_library_tab(storage: RepositoryStorage) -> None:
    ...
    st.markdown(
        '<div class="small-note">Файлы сохраняются в постоянное хранилище `storage/uploads`, разобранные токены — в `storage/parsed`, метаданные и выравнивания — в `storage/corpus.db`.</div>',
        unsafe_allow_html=True,
    )
    ...
    with left:
        if st.button("Импортировать XML из папки проекта", use_container_width=True):
            ...
            for xml_path in storage.scan_project_xml_files(BASE_DIR):
                before = len(storage.list_documents())
                storage.import_document_file(xml_path)
                after = len(storage.list_documents())
                if after > before:
                    imported += 1
                else:
                    skipped += 1
    ...
    with right:
        uploads = st.file_uploader(
            ...
            help="Поддерживаются XML-TEI в Unicode; приложение автоматически определяет фактическую кодировку файла.",
        )
        if uploads:
            imported = 0
            for upload in uploads:
                storage.import_document_bytes(file_name=upload.name, source_bytes=upload.getvalue(), display_name=upload.name)
                imported += 1
    ...
    for doc in documents:
        rows.append(
            {
                "ID": doc["doc_id"],
                "Список": doc["display_name"],
                "Название": doc["title"],
                "Листы": "–".join(doc["sheet_labels"]) if doc["sheet_labels"] else "нет листовой разметки",
                "Слов": doc["word_count"],
                "Кодировка": doc["actual_encoding"],
                "Предупреждения": " | ".join(doc["warnings"]) if doc["warnings"] else "",
            }
        )
...
def create_alignment_form(storage: RepositoryStorage, engine: AlignmentEngine) -> str | None:
    ...
    with st.form("create_alignment_form"):
        ...
        st.markdown('<div class="section-card">Диапазон листов главного списка</div>', unsafe_allow_html=True)
        master_start, master_end = choose_sheet_range(master_doc, "master")
        st.markdown('<div class="section-card">Подключаемые списки</div>', unsafe_allow_html=True)
...
def row_dataframe(state: dict, doc_map: dict[str, dict], visible_docs: list[str], variant_filter: list[str], row_span: tuple[int, int]) -> pd.DataFrame:
    rows = []
    for row in state["rows"]:
        ...
        if not (row_span[0] <= row["row_index"] <= row_span[1]):
            continue
        item = {"№": row["row_index"], "Тип": row["variant_type"]}
        for doc_id in visible_docs:
            item[doc_map[doc_id]["display_name"]] = cell_text(row["cells"].get(doc_id, []))
        rows.append(item)
...
def render_row_context(state: dict, doc_map: dict[str, dict], focus_row: int, visible_docs: list[str]) -> None:
    ...
    df = row_dataframe(state, doc_map, visible_docs, VARIANT_TYPES, (start, end))
...
def render_alignment_tab(storage: RepositoryStorage, engine: AlignmentEngine) -> None:
    ...
    selected_label = st.selectbox("Открыть сохранённое выравнивание", list(alignment_options), index=list(alignment_options).index(default_label))
    ...
    with top_left:
        card("Название", state["name"])
    with top_mid:
        card("Строк выравнивания", str(len(state["rows"])))
    with top_right:
        card("Главный список", doc_map[state["master_doc_id"]]["display_name"])

    st.markdown("### Порядок следования списков")
    order = state.get("visible_document_order", [state["master_doc_id"]] + [item["document_id"] for item in state["witnesses"]])
    doc_choice = st.selectbox("Переставить список", order, format_func=lambda doc_id: doc_map[doc_id]["display_name"])
    ...
    with c2:
        if st.button("Ниже", use_container_width=True):
            ...
            storage.save_alignment(state)
    with c3:
        st.caption("Текущий порядок: " + " → ".join(doc_map[doc_id]["display_name"] for doc_id in state["visible_document_order"]))
    ...
    table = row_dataframe(state, doc_map, visible_docs, variant_filter, row_span)
    st.dataframe(table, use_container_width=True, hide_index=True, height=520)

    st.markdown("### Статистика разночтений")
    st.dataframe(stats_dataframe(state), use_container_width=True, hide_index=True)

    st.markdown("### Ручная коррекция")
    focus_row = st.number_input("Строка для редактирования", min_value=1, max_value=max(1, len(state["rows"])), value=min(row_span[0], len(state["rows"])), step=1)
    render_row_context(state, doc_map, focus_row, visible_docs or state["visible_document_order"])

    editable_docs = state["visible_document_order"]
    selected_doc = st.selectbox("Какой список правим", editable_docs, format_func=lambda doc_id: doc_map[doc_id]["display_name"])
    ...
    with c1:
        manual_type = st.selectbox("Исправить тип разночтения", VARIANT_TYPES, index=VARIANT_TYPES.index(state["rows"][focus_row - 1]["variant_type"]))
    with c2:
...
def render_help_tab() -> None:
    ...
    st.markdown(
        """
        1. Импортируйте XML-TEI из папки проекта или через загрузку файлов.
        2. Создайте выравнивание: выберите главный список, диапазон его листов и соответствующие диапазоны других списков.
        3. После автоматического выравнивания используйте фильтры и блок ручной коррекции для исправления ошибок.
        4. Все результаты сразу сохраняются в постоянное хранилище, поэтому работу можно продолжить после закрытия приложения.
        5. Экспорт создаёт отдельный TEI-документ параллельного корпуса и не изменяет исходные рукописные файлы.
        """
    )
    st.markdown(
        """
        **Где что хранится**
        ...
        - `storage/uploads` — исходные загруженные XML.
        - `storage/parsed` — разобранные токены и метаданные, пригодные для повторного открытия без новой загрузки.
        - `storage/corpus.db` — документы, сохранённые выравнивания, ручные правки и ссылка на экспорт.
        - `storage/exports` — выгруженные TEI-файлы параллельного корпуса.
        """
    )
...
def main() -> None:
    ...
    st.markdown(
        ...
        <div class="section-card">
          <h1 style="margin-bottom:0.3rem;">Параллельный корпус списков</h1>
          <div class="small-note">
            Рабочее Streamlit-приложение для загрузки XML-TEI, автоматического выравнивания по словам,
            фиксации разночтений, ручной коррекции и экспорта результата обратно в TEI.
          </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    with col1:
        card("Импортировано списков", str(len(docs)))
    with col2:
        card("Сохранено выравниваний", str(len(storage.list_alignments())))
    with col3:
        word_total = sum(doc["word_count"] for doc in docs)
        card("Слов в хранилище", str(word_total))
